...
{
    /// <summary>
    /// Represents the default settings for Entity Framework Core.
    /// </summary>
    public class EfCoreSettings
    {
        ...
        /// <summary>
        /// Gets or sets the value indicating if database query logging is on or off.
        /// </summary>
        public LoggingBehavior LoggingBehavior { get; set; }

        /// <summary>
        /// Loads the <see cref="EfCoreSettings" /> from configuration.
        /// </summary>
        /// <param name="configuration">The configuration instance where the settings are loaded from.</param>
        /// <param name="sectionName">The name of the section that represents the EF Core settings.</param>
        ...
        public static EfCoreSettings FromConfiguration(IConfiguration configuration, string sectionName = DefaultSectionName) =>
            FromConfiguration<EfCoreSettings>(configuration, sectionName);

        /// <summary>
        /// Loads the EF Core settings from configuration.
        /// </summary>
        /// <typeparam name="T">The type of EF Core settings that will be used to load the settings.</typeparam>
        /// <param name="configuration">The configuration instance where the settings are loaded from.</param>
        /// <param name="sectionName">The name of the section that represents the EF Core settings.</param>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="configuration"/> or <paramref name="sectionName"/> is null.</exception>
        /// <exception cref="ArgumentException">Thrown when <paramref name="sectionName"/> is an empty string or contains only whitespace.</exception>
        ...
        public static T FromConfiguration<T>(IConfiguration configuration, string sectionName = DefaultSectionName)
        {
            ...
            return configuration.GetSection(sectionName)
                                .Get<T?>() ?? throw new InvalidConfigurationException($"EF Core settings could not be retrieved from configuration section \"{sectionName}\".");
        }
    }
}